...
class LinkEvaluator:
    ...
    """
    Responsible for evaluating links for a particular application.
    """
    ...
    def evaluate_link(self, link: Link) -> Tuple[bool, Optional[str]]:
        ...
        if link.egg_fragment:
            ...
            if ext == WHEEL_EXTENSION:
                ...
                if canonicalize_name(wheel.name) != self._canonical_name:
                    reason = "wrong application name (not {})".format(self.project_name)
                    return (False, reason)
        ...
        if not version:
            reason = f"Missing application version for {self.project_name}"
            return (False, reason)
...
def filter_unallowed_hashes(
    ...
    if not hashes:
        logger.debug(
            "Given no hashes to check %s links for application %r: "
    ...
    logger.debug(
        "Checked %s links for application %r against %s hashes "
...
class PackageFinder:
    ...
    def process_project_url(
        ...
        logger.debug(
            "Fetching application page and analyzing links: %s",
            project_url,
        )
    ...
    def find_best_candidate(
        ...
        """Find matches for the given application and specifier.
