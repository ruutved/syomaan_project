...
class LintMiddleware:
    ...
    .. code-block:: python
        ...
        application = LintMiddleware(application)
    """
    ...
    def __call__(self, *args: t.Any, **kwargs: t.Any) -> t.Iterable[bytes]:
        if len(args) != 2:
            warn("A WSGI application takes two arguments.", WSGIWarning, stacklevel=2)

        if kwargs:
            warn(
                "A WSGI application does not take keyword arguments.", WSGIWarning, stacklevel=2
            )
