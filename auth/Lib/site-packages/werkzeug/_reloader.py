...
def _iter_module_paths() -> t.Iterator[str]:
    ...
    # List is in case the value is modified by the application while updating.
...
def _find_stat_paths(
    ...
    such as a application root or ``sys.path.insert``, should be the paths
...
def _get_args_for_reloading() -> t.List[str]:
    ...
    if getattr(__main__, "__package__", None) is None or (
        ...
        # Executed a file, like "python application.py".
...
def run_with_reloader(
    ...
    try:
        if os.environ.get("WERKZEUG_RUN_MAIN") == "true":
            ...
            # the application thread and reloader update loop.
            with reloader:
                t.start()
                reloader.run()
        else:
